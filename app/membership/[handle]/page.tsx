...
export default function MembershipPage({params}:{params:Promise<{handle:string}>}) {
  ...
  return <div className="mx-auto max-w-6xl px-4 py-10 sm:px-8">
    <div className="text-center"><span className="brand-gradient inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold text-white"><Sparkles size={14}/>直接支持你喜欢的创作者</span><h1 className="mt-5 text-4xl font-black sm:text-5xl">加入 {creator.name} 的会员</h1><p className="mx-auto mt-3 max-w-2xl muted">持续获得独家作品、幕后过程和更近距离的交流。随时可以取消。</p></div>

...
export default function CreatorPage({params}:{params:Promise<{handle:string}>}) {
  ...
  useEffect(()=>{fetch(`/api/creators/${handle}`).then(response=>response.ok?response.json():null).then(body=>{if(body?.creator){setFollowing(Boolean(body.creator.following));setFollowerCount(body.creator.followers)}}).catch(()=>undefined)},[handle]);
  if(!creator)return <div className="p-20 text-center">博主不存在</div>;
  const updateFollow=async()=>{
    if(!session?.user)return window.location.assign(`/sign-in?callbackUrl=${encodeURIComponent(window.location.pathname)}`);
    const next=!following; setFollowing(next); setFollowerCount(value=>value+(next?1:-1));
    const response=await fetch(`/api/creators/${handle}/follow`,{method:next?"POST":"DELETE"});
    if(!response.ok){setFollowing(!next);setFollowerCount(value=>value+(next?-1:1))}
  };
  ...
  return <div>
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-8">
      <section className="glass relative -mt-20 rounded-[34px] p-6 shadow-soft sm:p-9">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end"><div className="rounded-full border-4 border-white dark:border-[#211d24]"><Avatar text={creator.avatar}/></div><div className="flex-1"><h1 className="flex items-center gap-2 text-3xl font-black">{creator.name}{creator.verified&&<BadgeCheck className="text-violet"/>}</h1><p className="mt-1 muted">@{creator.handle} · {creator.category}</p><p className="mt-3 max-w-2xl leading-7">{creator.bio}</p></div><div className="flex gap-2"><button onClick={updateFollow} className="rounded-full border border-[var(--line)] px-5 py-3 text-sm font-bold"><BellRing size={16} className="mr-2 inline"/>{following?"已关注":"关注"}</button><Link href={`/membership/${creator.handle}`} className="brand-gradient rounded-full px-5 py-3 text-sm font-bold text-white">加入会员</Link></div></div>
        <div className="mt-7 flex gap-8 border-t border-[var(--line)] pt-5"><p><b>{(followerCount/10000).toFixed(1)}万</b><span className="ml-1 text-sm muted">关注者</span></p><p><b>{creator.members+store.subscriptions.filter(s=>s.creatorId===creator.id).length}</b><span className="ml-1 text-sm muted">会员</span></p><p><b>{creatorPosts.length}</b><span className="ml-1 text-sm muted">作品</span></p></div>
      </section>

...
test("phase 6 server authorization rejects spoofed identities and role headers", async ({ request }, testInfo) => {
  ...
  try {
    ...
    const ownedPostBody = await ownedPost.json();
    ownedPostId = ownedPostBody.post.id;
    expect(ownedPostBody.post.creatorId).toBe("c1");
...
test("phase 6 comments require a session and keep the session author", async ({ request }, testInfo) => {
  await requirePhase6(request, testInfo);

...
function createMediaDatabase(asset: FakeAsset) {
  ...
  return {
    records,
    mediaAsset: {
      async findMany(input: { where: { status: string }; take: number }) {
      ...
      async updateMany(input: {
        ...
        for (const record of records.values()) {
          ...
          Object.assign(record, input.data, { updatedAt: new Date() });
          count += 1;
        }
        return { count };
      },
      async count(input: { where: { id?: string; status?: string } }) {
...
test("overlapping media processors exclusively claim an asset so the loser never deletes the winner derivative", async () => {
  ...
  const derivativeKey = `derivatives/${asset.id}/watermarked.jpg`;
...
test("media processing recovers only stale claims after the bounded lease", async () => {
  ...
  expect(activeDatabase.records.get(activeAsset.id)?.status).toBe("processing_claimed");
});
...
test("PostgreSQL grants exactly one exclusive claim for concurrent attempts", async ({}, testInfo) => {
  ...
  try {
    ...
    expect(await prisma.mediaAsset.findUnique({
      where: { id: assetId },
      select: { status: true }
    })).toEqual({ status: "processing_claimed" });
  } finally {
